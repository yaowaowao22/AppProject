...
#
# ⚠️ Vendored from expo-mobile-builder skill.
#    Canonical source: ~/.claude/skills/expo-mobile-builder/scripts/detect-native-change.py
#
#    スキル側に修正が入ったら `cp` で resync すること。
#    プロジェクト固有の調整が必要な時はこのファイルを直接編集するが、
#    その変更は最終的にスキル側にも反映して他プロジェクトで再利用可能にする。
#
# 目的:
