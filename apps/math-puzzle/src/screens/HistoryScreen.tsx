...
export function HistoryScreen() {
  ...
  const [history, , loading] = useLocalStorage<GameResult[]>('math_puzzle_history', []);

  const renderItem = ({ item }: { item: GameResult }) => (
    <ListItem
      title={`${getDifficultyLabel(item.difficulty)} - 目標: ${item.target}`}
      subtitle={`${formatDate(item.date)} / ${formatTime(item.timeSeconds)}`}
      rightElement={
        <Badge
          label={item.solved ? 'クリア' : '未クリア'}
          variant={item.solved ? 'success' : 'default'}
        />
      }
    />
  );
