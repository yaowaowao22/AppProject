...
export function HistoryScreen() {
  ...
  const renderItem = ({ item }: { item: MazeResult }) => (
    <View>
      <ListItem
        title={getDifficultyLabel(item.difficulty)}
        subtitle={`${formatTime(item.timeSeconds)} | ${item.moves}歩 | ${formatDate(item.date)}`}
        leftIcon="🏆"
      />
      <Divider />
    </View>
