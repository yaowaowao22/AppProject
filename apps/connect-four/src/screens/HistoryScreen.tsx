...
import { getDifficultyLabel } from '../utils/connectFour';
import type { GameResult } from '../utils/connectFour';
...
export function HistoryScreen() {
  ...
  const [history, , loading] = useLocalStorage<GameResult[]>('connect_four_history', []);
  ...
  const renderItem = ({ item }: { item: GameResult }) => (
    <ListItem
      title={`${getDifficultyLabel(item.difficulty)} - ${item.moves}手`}
