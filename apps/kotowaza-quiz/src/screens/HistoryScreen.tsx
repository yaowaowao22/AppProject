...
import { useTheme, H2, Body, Caption, ListItem, Divider, EmptyState, Button, Badge } from '@massapp/ui';
...
export function HistoryScreen() {
  ...
  const renderItem = useCallback(
    ({ item }: { item: QuizResult }) => {
      ...
      return (
        <ListItem
          title={`${item.level} / ${item.mode}  ${item.correct}/${item.total}問正解`}
      ...
      <ScreenWrapper>
        <View style={[styles.emptyContainer, { padding: spacing.lg }]}>
          <EmptyState
            icon="🏆"
            title="まだ記録がありません"
            subtitle="ことわざクイズをプレイして記録を残しましょう"
          />
        </View>
      </ScreenWrapper>
    );
