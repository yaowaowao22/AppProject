...
import {
  ...
  CATEGORIES,
  CATEGORY_ICONS,
} from '../data/emojiQuestions';
import type { EmojiQuestion, Category } from '../data/emojiQuestions';
import type { QuizResult } from './TitleScreen';

type GamePhase = 'setup' | 'playing' | 'feedback' | 'result';
type QuizCount = 10 | 15 | 20;
...
export function GameScreen() {
  ...
  const handleStartQuiz = useCallback(() => {
    ...
    const q = getQuestions(categoryFilter, selectedCount);
    if (q.length === 0) return;

    const generatedChoices = q.map((question) =>
      generateChoices(question, emojiQuestions)
    );
  ...
  const handleAnswer = useCallback(
    (choiceIndex: number) => {
      ...
      const correct = currentChoices[choiceIndex] === question.answer;
    ...
    [selectedAnswer, questions, choices, currentIndex, flashAnim]
  );
  ...
  const handleNext = useCallback(() => {
    ...
    if (nextIndex >= questions.length) {
      ...
      const result: QuizResult = {
        ...
        category: selectedCategory ?? '全カテゴリ',
        correct: score,
          ...
          <View style={styles.resultContainer}>
            ...
            <Card style={[styles.resultCard, { marginBottom: spacing.lg }]}>
              <View style={styles.resultRow}>
                <Body color={colors.textSecondary}>カテゴリ</Body>
                <H3>{selectedCategory ?? '全カテゴリ'}</H3>
              </View>
              <View
                style={[
  ...
  const correctAnswerIndex = currentChoices.indexOf(currentQuestion?.answer);
      ...
      >
        ...
        {currentQuestion && (
          <View style={{ alignItems: 'center', marginBottom: spacing.sm }}>
            <Caption color={colors.textMuted}>
              {CATEGORY_ICONS[currentQuestion.category as Category]}{' '}
        ...
        {currentQuestion && (
          <>
            ...
            {/* Feedback: show correct answer + hint */}
            {phase === 'feedback' && (
              <Card style={[styles.explanationCard, { marginTop: spacing.md }]}>
                ...
                >
                  {currentQuestion.emojis} = {currentQuestion.answer}
                  {'\n'}
