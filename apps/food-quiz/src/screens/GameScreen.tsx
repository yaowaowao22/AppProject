...
const modeLabels: Record<QuizMode, string> = {
  ingredientToFood: '\u98DF\u6750\u2192\u6599\u7406',
  emojiToName: '\u7D75\u6587\u5B57\u2192\u540D\u524D',
  originCountry: '\u539F\u7523\u56FD',
  calorieCompare: '\u30AB\u30ED\u30EA\u30FC\u30AF\u30A4\u30BA',
  triviaOX: '\u8C46\u77E5\u8B58\u25CB\u00D7',
};
...
export function GameScreen() {
  ...
  const [history, setHistory] = useLocalStorage<QuizResult[]>('food-quiz-history', []);

  const mode: QuizMode = route.params?.mode ?? 'emojiToName';
  ...
  const startNewQuiz = () => {
    const generated = generateQuiz(mode, count);
  ...
  }, [route.params?.mode, route.params?.count, route.params?.timestamp]);
  ...
  const isTriviaMode = mode === 'triviaOX';
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }}>
        <View style={[styles.container, { padding: spacing.lg }]}>
          <View style={[styles.header, { marginBottom: spacing.md }]}>
            <Caption>{modeLabels[mode]}</Caption>
          ...
          >
            ...
            <H3 align="center" color={isTriviaMode ? colors.text : colors.textSecondary}>
          ...
          <View
            style={[
              isTriviaMode ? styles.triviaChoicesContainer : styles.choicesContainer,
              { gap: spacing.sm },
            ]}
          >
            {currentQuestion.choices.map((choice, index) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  style={[
                    isTriviaMode ? styles.triviaChoiceButton : styles.choiceButton,
                    {
