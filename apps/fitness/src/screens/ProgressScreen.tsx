...
function getWeekStart(date: Date): Date {
  ...
  const diff = dow === 0 ? -6 : 1 - dow; // 月曜始まめE  d.setDate(d.getDate() + diff);
  return d;
}
...
export default function ProgressScreen() {
  ...
  const calendarData = useMemo(() => {
    ...
    const startOffset = dow === 0 ? 6 : dow - 1; // 月曜始まめE
    ...
    return { days, label: `${year}年${month + 1}朁E };
  ...
  const onPRCardPress = (exerciseId: string) => {
    ...
    const title = ex.name;
    const subtitle = pr.maxWeight !== null ? `自己ベスチE${pr.maxWeight}kg` : '記録なぁE;
    ...
    const rows = pastSessions
      ...
      .map((s, i) => {
        const d = new Date(s.date + 'T00:00:00');
        const label = `${d.getMonth() + 1}朁E{d.getDate()}日`;
        ...
        return {
          label,
          detail: `${s.sets.length}セチE��`,
          value: maxW !== null ? `${maxW}kg` : '自釁E,
          badge: hasPR,
          onPress: workoutId
  ...
  return (
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScreenHeader title="進捁E showHamburger />
    <ScrollView
    ...
    >
      ...
      {/* ── Section 1: PRカーチE── */}
      <Text style={styles.sectionLabel}>自己ベスチE/Text>
      {prCards.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              accessibilityLabel={`${item.name} 自己ベスチE${item.maxWeight}kg`}
            >
      ...
      {/* ── Section 2: 部位別ベスト�EリュームカーチE── */}
      <Text style={styles.sectionLabel}>部位別ベスチE/Text>
      <FlatList
        horizontal
        data={partVolumes}
        ...
        renderItem={({ item }) => (
          <View
            style={styles.pvCard}
            accessibilityLabel={`${item.label} ベスト�Eリューム ${item.volume > 0 ? `${item.volume}kg` : 'なぁE}`}
          >
