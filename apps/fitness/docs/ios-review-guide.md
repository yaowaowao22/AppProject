...
| 8.11 | fitness-api デプロイ | ⚠️ | `cd server/fitness-api && npx wrangler deploy` — PP/Support URL のために必須 |
...
```
[ ] 1. fitness-api デプロイ完了 → Privacy Policy URL 疎通確認
[ ] 2. ASC でアプリ作成 → ascAppId 取得 → eas.json に設定
[ ] 3. ASC メタデータ設定（説明文・キーワード・カテゴリ・年齢制限・URL）
[ ] 4. スクリーンショット生成・アップロード（3サイズ × 4枚）
[ ] 5. App Privacy Labels を「Data Not Collected」に設定
[ ] 6. アイコン画像確認（1024×1024 / 透過なし / 角丸なし）
[ ] 7. eas build --platform ios --profile production 実行
[ ] 8. TestFlight で動作確認
[ ] 9. eas submit --platform ios --profile production 実行
[ ] 10. 審査サブミッション（ASC から「Submit for Review」）
```
