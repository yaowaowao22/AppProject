...
import { getDifficultyLabel } from '../utils/gomoku';
...
export function HistoryScreen() {
  ...
  const [history, , loading] = useLocalStorage<GameResult[]>('gomoku_history', []);
  ...
  const renderItem = ({ item }: { item: GameResult }) => (
    <ListItem
      title={`${getDifficultyLabel(item.difficulty)} - ${item.totalMoves}手`}
